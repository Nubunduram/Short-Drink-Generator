...
let eauDeVie: HTMLSpanElement = document.querySelector("#strong")!;
let liquorSirupElement: HTMLSpanElement = document.querySelector("#sweet")!;
const button: HTMLButtonElement = document.querySelector("#generate-button")!;
  
const listEDV: string[] = [
    "Calvados",
    "Cachaca",
    "Cognac",
    "Gin",
...
function getRandomLiquorSirup(): string {
    const randomNumberForSweet: number = Math.floor(Math.random() * listLiquorSirup.length);
    return listLiquorSirup[randomNumberForSweet];
}

function getRandomEDV(): string {
    const randomNumberForEDV: number = Math.floor(Math.random() * listEDV.length);
